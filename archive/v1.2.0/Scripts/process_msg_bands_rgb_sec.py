...
if (natural_color_sun == True): composites.append('natural_color_sun')
if (night_fog == True): composites.append('night_fog')
if (overview_sun == True): composites.append('overview_sun')
...
decompress_files = glob(XRIT_DECOMPRESS_OUTDIR + '*MSG*')
...
result = re.search('________-_________-EPI______-(.*)-__', path_proc)
path2 = re.sub('H-000*(.*)', '', path_proc)
...
if (airmass == True):	
    ...
    product = 'ARMRGB'
    type = 'RGB'
    data = get_enhanced_image(local_scn[variable])
...
if (natural_color_sun == True):
    variable = 'natural_color_sun'
    product = 'DLCRGB'	
    type = 'RGB'
    data = get_enhanced_image(local_scn[variable]) 
    data = data.finalize(fill_value=None)[0]
...
if (overview_sun == True):
    variable = 'overview_sun'
    product = 'OVWRGB'	
    type = 'RGB'
    data = get_enhanced_image(local_scn[variable]) 
    data = data.finalize(fill_value=None)[0]
