...
IF EXIST %ParentFolderName%\Miniconda3\ (
	...
	start /wait "" %~dp0/Miniconda3/Miniconda3-latest-Windows-x86_64.exe /InstallationType=JustMe /RegisterPython=0 /S /D=%ParentFolderName%\Miniconda3
...
IF EXIST %ParentFolderName%\Miniconda3\envs\showcast\ (
	...
	:: if not, update conda and create the showcast env
	echo:
	echo Updating conda... [this may take some minutes]
	echo:
	call %UserProfile%\Miniconda3/condabin/conda update -n base -c defaults conda --yes
	echo:
	echo Creating the SHOWCast environment... [this will take some minutes]
	echo:
	call %ParentFolderName%\Miniconda3\condabin\conda create --name showcast -c conda-forge affine cartopy folium gdal geopandas glymur matplotlib metpy netcdf4 pandas Pillow pygrib pyhdf pyorbital pyproj pyresample rasterstats satpy siphon --yes
)
