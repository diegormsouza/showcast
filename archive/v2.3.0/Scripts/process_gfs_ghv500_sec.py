...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=False, zorder=7)
