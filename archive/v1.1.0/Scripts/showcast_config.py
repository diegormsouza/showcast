...
g16_trurgb = True # GOES-16 True Color RGB
...
# METEOSAT PRODUCTS
msg_produc = True # METEOSAT Bands and RGB's
...
############################################################
# GOES-16 True Color RGB
############################################################
if g16_trurgb:
    max_files = 1
    extent = [-105.0, -60.0, -15.0, 20.0]
    resolution = 4 # Max Res.: 2 km / NOTE: This is processing demanding! If using 2 km, please reduce the area! 
    prod_dir = gnc_dir + 'GOES-R-RGB-Composites//'
    identifier = 'G16_NTCS07_*.tif'
    composite = '_TRU'
    script = python_env + 'python process_g16_rgb_true.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
############################################################
# METEOSAT - 0 Degree (IMAGERY AND RGB'S)
############################################################
if msg_produc:
    max_files = 1
    ...
    composite = ''
    script = python_env + 'python process_msg_bands_rgb.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
