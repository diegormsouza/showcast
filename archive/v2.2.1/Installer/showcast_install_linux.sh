...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	if [[ $REPLY =~ ^[Yy]$ ]]; then
		...
		if [ -d "$parentdir/Miniconda3/envs/showcast" ]; then
			...
			$parentdir/Miniconda3/condabin/conda create --name showcast -c conda-forge affine cartopy folium gdal geopandas glymur matplotlib netcdf4 pandas Pillow pygrib pyhdf pyorbital pyproj pyresample rasterstats satpy siphon --yes		
		fi		
	fi
fi
echo
