...
path_ch09 = path_ch09[:-4]

# Get the Band 10, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch10 = re.sub('Band09\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch09)
else:
    path_ch10 = re.sub('Band09/OR_ABI-L2-CMIPF*(.*)', '', path_ch09)

# Get the start of scan from the file name
scan_start = (path_ch09[path_ch09.find("_s")+2:path_ch09.find("_e")])

#------------------------------------------------------------------------------------------------------

file = []
for filename in sorted(glob.glob(path_ch10+'Band10//OR_ABI-L2-CMIPF-M*C10_G1*.nc')):
...
osystem = platform.system()
if osystem == "Windows":
    path_ch13 = re.sub('Band09\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch09)
else:
    path_ch13 = re.sub('Band09/OR_ABI-L2-CMIPF*(.*)', '', path_ch09)
...
# Get the Band 11, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch11 = re.sub('Band09\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch09)
else:
    path_ch11 = re.sub('Band09/OR_ABI-L2-CMIPF*(.*)', '', path_ch09)
...
file = []
for filename in sorted(glob.glob(path_ch11+'Band11//OR_ABI-L2-CMIPF-M*C11_G1*.nc')):
    file.append(filename)
...
#------------------------------------------------------------------------------------------------------
# Read the image
file_ch09 = Dataset(path_ch09)

# Read the satellite 
satellite = getattr(file_ch09, 'platform_ID')

# Read the band number
band = str(file_ch09.variables['band_id'][0]).zfill(2)

# Desired resolution
resolution = int(sys.argv[6])

# Read the resolution
band_resolution_km = getattr(file_ch09, 'spatial_resolution')
...
longitude = file_ch09.variables['goes_imager_projection'].longitude_of_projection_origin

# Read the semi major axis
a = file_ch09.variables['goes_imager_projection'].semi_major_axis

# Read the semi minor axis
b = file_ch09.variables['goes_imager_projection'].semi_minor_axis

# Calculate the image extent 
h = file_ch09.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch09.variables['x_image_bounds'][0] * h 
x2 = file_ch09.variables['x_image_bounds'][1] * h 
y1 = file_ch09.variables['y_image_bounds'][1] * h 
y2 = file_ch09.variables['y_image_bounds'][0] * h 

# Getting the file time and date
add_seconds = int(file_ch09.variables['time_bounds'][0])
...
grid = remap(path_ch09, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
     
# Read the data returned by the function 
data_ch09 = grid.ReadAsArray() - 273.15
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Read the image
...
# Call the reprojection funcion
grid = remap(path_ch10, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
...
#------------------------------------------------------------------------------------------------------
# Read the image
file_ch11 = Dataset(path_ch11)

# Variable to remap
variable = "CMI"

# Call the reprojection funcion
grid = remap(path_ch11, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
     
# Read the data returned by the function 
data_ch11 = grid.ReadAsArray() - 273.15
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# RGB Components
...
"dpi": 150, 
"states_color": 'white', "states_width": data_ch09.shape[0] * 0.00018, 
"countries_color": 'white', "countries_width": data_ch09.shape[0] * 0.00012,
"continents_color": 'white', "continents_width": data_ch09.shape[0] * 0.00025,
"grid_color": 'white', "grid_width": data_ch09.shape[0] * 0.00025, "grid_interval": 10.0,
"vmin": 0, "vmax": 1, "cmap": 'jet',
"title_text": "GOES-" + satellite[1:3] + " SO2 RGB ", "title_size": int(data_ch09.shape[1] * 0.005), "title_x_offset": int(data_ch09.shape[1] * 0.01), "title_y_offset": data_ch09.shape[0] - int(data_ch09.shape[0] * 0.016), 
"thick_interval": 0, "cbar_labelsize": int(data_ch09.shape[0] * 0.005), "cbar_labelpad": -int(data_ch09.shape[0] * 0.0),
"file_name_id_1": satellite,  "file_name_id_2": product
}
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Choose the plot size (width x height, in inches)
fig = plt.figure(figsize=(data_ch09.shape[1]/float(plot_config["dpi"]), data_ch09.shape[0]/float(plot_config["dpi"])), dpi=plot_config["dpi"])
...
my_legend = plt.imread(main_dir + '//Legends//AIRMASS_legend.png')
