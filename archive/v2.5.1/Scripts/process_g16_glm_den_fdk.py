...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        heatmap, xedges, yedges = np.histogram2d(e_lons, e_lats, bins=1000)
        ...
        colors = plt.get_cmap('Wistia_r')(range(256))
        ...
        "title_text": "GOES-16 GLM (Density) + BAND-13", "title_size": int(data.shape[1] * 0.005), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
        ...
        glm = ax.imshow(heatmap, vmin=0, vmax=6, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=ccrs.PlateCarree(), alpha = 1.00, zorder=4)

        # Plot the accumulated flashes, groups and events
        img2 = ax.plot(e_lons,e_lats, 'o', markersize=0.5, color='#fffb8a', alpha = 0.01, transform=ccrs.PlateCarree(), zorder=5)
        img3 = ax.plot(g_lons,g_lats, 'o', markersize=0.5, color='#fffdc7', alpha = 0.02, transform=ccrs.PlateCarree(), zorder=6)
        img4 = ax.plot(f_lons,f_lats, 'o', markersize=0.5, color='#fff700', alpha = 0.20, transform=ccrs.PlateCarree(), zorder=7)
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
