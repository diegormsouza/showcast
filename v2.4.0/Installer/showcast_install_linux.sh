...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	if [ -d "$parentdir/Miniconda3" ]; then
		...
		echo
		sh ./Miniconda3/Miniconda3-py39_4.9.2-Linux-x86_64.sh -b -p $parentdir/Miniconda3
	...
	if [[ $REPLY =~ ^[Yy]$ ]]; then
		...
		if [ -d "$parentdir/Miniconda3/envs/showcast" ]; then
			...
			$parentdir/Miniconda3/condabin/conda create --name showcast -c conda-forge affine cartopy folium gdal geopandas glymur matplotlib metpy netcdf4 pandas Pillow pygrib pyhdf pyorbital pyproj pyresample rasterstats satpy siphon --yes		
		fi		
	fi
fi
echo
