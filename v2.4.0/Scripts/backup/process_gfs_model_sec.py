...
print("Forecast Hour: ", grb.forecastTime)
...
# Scale / Convert the data if needed
data = data - 272.15 # K to C

# Define de contour interval
data_min =  -5 #np.min(data) 
data_max = 40 #np.max(data) 
interval = 2
levels = np.arange(data_min,data_max,interval)
 
# Plot the image
#img = ax.imshow(data, extent=img_extent, origin='lower', cmap='GnBu', zorder=1)
img1 = ax.contourf(lons, lats, data, transform=ccrs.PlateCarree(), cmap='rainbow', levels=levels)
...
plt.title("Init: " + str(grb.analDate)[:-6] + "Z | Forecast Hour: [" + str(grb.forecastTime).zfill(3) + "]" + " | Valid: " + str(grb.validDate)[:-6] + "Z ", fontsize=10, loc='right')
...
# Save the image
plt.savefig('Image_Test_Temp.png', bbox_inches='tight', pad_inches=0)
