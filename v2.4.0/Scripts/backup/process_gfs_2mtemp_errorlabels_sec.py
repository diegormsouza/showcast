...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    path = path_gfs + str(hour).zfill(3)
    print(path)
    ...
    if (os.path.exists(path)):
        ...
        "resolution": 0, 
        "dpi": 150, 
        "states_color": 'black', "states_width": tmtmp.shape[0] * 10 * 0.00006, 
        "countries_color": 'black', "countries_width": tmtmp.shape[0] * 10 * 0.00012,
        "continents_color": 'black', "continents_width": tmtmp.shape[0] * 10 * 0.00025,
        "grid_color": 'white', "grid_width": tmtmp.shape[0] * 10 * 0.00025, "grid_interval": 10.0,
        "vmin": vmin, "vmax": vmax, "cmap": cmap,
        "title_text": "GFS (0.5°): 2 m Temperature (°C)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": int(tmtmp.shape[1] * 10 * 0.004), "title_x_offset": int(tmtmp.shape[1] * 10 * 0.02), "title_y_offset": tmtmp.shape[0] * 10 - int(tmtmp.shape[0] * 10 * 0.032), 
        "thick_interval": thick_interval, "cbar_labelsize": int(tmtmp.shape[0] * 10 * 0.005), "cbar_labelpad": -int(tmtmp.shape[0] * 10 * 0.00),
        ...
        fig = plt.figure(figsize=(tmtmp.shape[1] * 10 /float(plot_config["dpi"]), tmtmp.shape[0] * 10 /float(plot_config["dpi"])), dpi=plot_config["dpi"])
        ...
        # Plot the image
        img1 = ax.contourf(lons, lats, tmtmp, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='both', zorder=1)    
        img2 = ax.contour(lons, lats, tmtmp, transform=ccrs.PlateCarree(), colors='black', linewidths=0.1, levels=levels, zorder=2)
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=True, zorder=11)
        ...
        ticks = ticks[1:]
        cb = fig.colorbar(img1, label="2 m Temperature (°C)", cax=axins1, orientation="vertical", ticks=ticks)
        ...
        conf.read(main_dir + '//Utils//Labels//labels_gfs.ini')
