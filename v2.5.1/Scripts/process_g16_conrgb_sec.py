...
path_ch08 = path_ch08[:-4]

# Get the Band 15, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch10 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch10 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)

# Get the start of scan from the file name
scan_start = (path_ch08[path_ch08.find("_s")+2:path_ch08.find("_e")])
...
osystem = platform.system()
if osystem == "Windows":
    path_ch07 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch07 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
...
osystem = platform.system()
if osystem == "Windows":
    path_ch13 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch13 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
...
osystem = platform.system()
if osystem == "Windows":
    path_ch05 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch05 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
...
osystem = platform.system()
if osystem == "Windows":
    path_ch02 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch02 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
...
band = str(file_ch08.variables['band_id'][0]).zfill(2)
...
h = file_ch08.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch08.variables['x_image_bounds'][0] * h 
x2 = file_ch08.variables['x_image_bounds'][1] * h 
y1 = file_ch08.variables['y_image_bounds'][1] * h 
y2 = file_ch08.variables['y_image_bounds'][0] * h 
...
grid = remap(path_ch08, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
...
G = data_ch07 - data_ch13
B = data_ch05 - data_ch02
...
"dpi": 150, 
"states_color": 'white', "states_width": data_ch08.shape[0] * 0.00018, 
"countries_color": 'white', "countries_width": data_ch08.shape[0] * 0.00012,
"continents_color": 'white', "continents_width": data_ch08.shape[0] * 0.00025,
"grid_color": 'white', "grid_width": data_ch08.shape[0] * 0.00025, "grid_interval": 10.0,
"vmin": 0, "vmax": 1, "cmap": 'jet',
"title_text": "GOES-" + satellite[1:3] + " CON RGB ", "title_size": int(data_ch08.shape[1] * 0.005), "title_x_offset": int(data_ch08.shape[1] * 0.01), "title_y_offset": data_ch08.shape[0] - int(data_ch08.shape[0] * 0.016), 
"thick_interval": 0, "cbar_labelsize": int(data_ch08.shape[0] * 0.005), "cbar_labelpad": -int(data_ch08.shape[0] * 0.0),
"file_name_id_1": satellite,  "file_name_id_2": product
}
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Choose the plot size (width x height, in inches)
fig = plt.figure(figsize=(data_ch08.shape[1]/float(plot_config["dpi"]), data_ch08.shape[0]/float(plot_config["dpi"])), dpi=plot_config["dpi"])
...
my_legend = plt.imread(main_dir + '//Legends//DAYCONVECTION_legend.png')
newax = fig.add_axes([0.50, 0.50, 0.49, 0.49], anchor='NE', zorder=13) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
