...
path_ch13_17 = path_ch13_17[:-4]

# Get the Band 15, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch13_16 = re.sub('Band13\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch13_17)
    path_ch13_16 = re.sub('GOES-S-CMI-Imagery//', '', path_ch13_16)
else:
    path_ch13_16 = re.sub('Band13/OR_ABI-L2-CMIPF*(.*)', '', path_ch13_17)
    path_ch13_16 = re.sub('GOES-S-CMI-Imagery//', '', path_ch13_16)
    print(path_ch13_16)
...
scan_start = (path_ch13_17[path_ch13_17.find("_s")+2:path_ch13_17.find("_e")])
scan_start = scan_start[0:11]

print(path_ch13_16+'GOES-R-CMI-Imagery//Band13//OR_ABI-L2-CMIPF-M*C13_G1*.nc')
print(scan_start)

file = []
for filename in sorted(glob.glob(path_ch13_16+'GOES-R-CMI-Imagery//Band13//OR_ABI-L2-CMIPF-M*C13_G1*.nc')):
...
extent_16 = [-106.29, -81.32, 6.29, 81.32]
extent_17 = [-216.29, -81.32, -106.29, 81.32]
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------

# Read the image
file = Dataset(path_ch13_17)
...
grid = remap(path_ch13_17, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
     
# Read the data returned by the function 
data_17 = grid.ReadAsArray()
data_17[data_17 == min(data_17[0])] = np.nan
...
# Call the reprojection funcion
grid = remap(path_ch13_16, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
     
# Read the data returned by the function 
data_16 = grid.ReadAsArray()
data_16[data_16 == min(data_16[0])] = np.nan
...
"dpi": 150, 
"states_color": 'white', "states_width": (data_17.shape[0]) * 0.00018, 
"countries_color": 'gold', "countries_width": (data_17.shape[0]) * 0.00012,
"continents_color": 'gold', "continents_width": (data_17.shape[0]) * 0.00025,
"grid_color": 'white', "grid_width": (data_17.shape[0]) * 0.00025, "grid_interval": 10.0,
"vmin": vmin, "vmax": vmax, "cmap": cmap,
"title_text": "GOES-16 and GOES-17" + " Band " + band + " Mosaic", "title_size": int((data_17.shape[1]+data_16.shape[1]) * 0.003), "title_x_offset": int((data_17.shape[1]+data_16.shape[1]) * 0.01), "title_y_offset": (data_17.shape[0]) - int((data_17.shape[0]) * 0.038), 
"thick_interval": thick_interval, "cbar_labelsize": int((data_17.shape[0]) * 0.005), "cbar_labelpad": -int((data_17.shape[0]) * 0),
"file_name_id_1": "G1X",  "file_name_id_2": "B13MOS_SEC" 
}
#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
# Choose the plot size (width x height, in inches)
fig = plt.figure(figsize=((data_17.shape[1]+data_16.shape[1])/float(plot_config["dpi"]), (data_17.shape[0])/float(plot_config["dpi"])), dpi=plot_config["dpi"])
...
img_extent_17 = [extent_17[0]+cent_lon, extent_17[2]+cent_lon, extent_17[1], extent_17[3]]
img_extent_16 = [extent_16[0]+cent_lon, extent_16[2]+cent_lon, extent_16[1], extent_16[3]]
...
data_171 = data_17
data_171 = np.maximum(data_171, 90)
data_171 = np.minimum(data_171, 313)
# Normalize the channel between a range
data_171 = (data_171-90)/(313-90)
# Invert colors
data_171 = 1 - data_171
img1 = ax.imshow(data_171, cmap='gray', vmin=0.1, vmax=0.25, alpha = 0.6, origin='upper', extent=img_extent_17, zorder=3)

#print("Second layer...")
# SECOND LAYER
data_172 = data_171
data_172[data_172 < 0.20] = np.nan
img2 = ax.imshow(data_172, cmap='gray', vmin=0.15, vmax=0.30, alpha = 1.0, origin='upper', extent=img_extent_17, zorder=4)

#print("Third layer...")
# THIRD LAYER
data_173 = data_17
data_173 = data_17 - 273.15
data_173[np.logical_or(data_173 < -80, data_173 > -28)] = np.nan
img3 = ax.imshow(data_173, cmap=cmap, vmin=-103, vmax=84, alpha=1.0, origin='upper', extent=img_extent_17, zorder=5)

#print("First layer...")
# Apply range limits for clean IR channel
data_161 = data_16
data_161 = np.maximum(data_161, 90)
data_161 = np.minimum(data_161, 313)
# Normalize the channel between a range
data_161 = (data_161-90)/(313-90)
# Invert colors
data_161 = 1 - data_161
img1 = ax.imshow(data_161, cmap='gray', vmin=0.1, vmax=0.25, alpha = 0.6, origin='upper', extent=img_extent_16, zorder=3)

#print("Second layer...")
# SECOND LAYER
data_162 = data_161
data_162[data_162 < 0.20] = np.nan
img2 = ax.imshow(data_162, cmap='gray', vmin=0.15, vmax=0.30, alpha = 1.0, origin='upper', extent=img_extent_16, zorder=4)

#print("Third layer...")
# THIRD LAYER
data_163 = data_16
data_163 = data_16 - 273.15
data_163[np.logical_or(data_163 < -80, data_163 > -28)] = np.nan
img3 = ax.imshow(data_163, cmap=cmap, vmin=-103, vmax=84, alpha=1.0, origin='upper', extent=img_extent_16, zorder=5)
