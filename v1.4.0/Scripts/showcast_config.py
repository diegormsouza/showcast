...
g16_b05rgb_fdk = True # GOES-16 L2 CMI - Band 05 - FULL DISK 
g16_b05rgb_sec = True # GOES-16 L2 CMI - Band 05 - USER SECTOR
...
g1X_b13mos_sec = True # GOES-16 and GOES-17 Band 13 Mosaic
...
gcm_icepro_sec = True # GCOM-W1 - SEA ICE NH AND SEA ICE SH
gcm_snowpr_sec = True # GCOM-W1 - SNOW COVER, SNOW DEPTH, SWE

# SEA ICE PRODUCTS
mul_sitype_sec = True # Sea Ice Type (NH and SH)
mul_siedge_sec = True # Sea Ice Edge (NH and SH)
dms_siconc_sec = True # Sea Ice Concentration - SSMIS and AMSR (NH and SH)
dms_siemis_sec = True # Sea Ice Emissivity - SSMIS and AMSU (NH and SH) 
...
#######################################################################################################
# GOES-16 Band 01 from RGB - FULL DISK
#######################################################################################################
if g16_b01rgb_fdk:
    max_files = 2
    resolution = 8 # Max Res.: 2 km 
    ...
    identifier = 'G16_NTCS07_*.tif'
    composite = '_B01'
    script = python_env + 'python process_g16_b01_rgb_fdk.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)

#######################################################################################################
# GOES-16 Band 01 from RGB - USER SECTOR
#######################################################################################################
if g16_b01rgb_sec:
    max_files = 2
    extent = [-63.0, -35.0, -35.0, -10.0] # [min_lon, min_lat, max_lon, max_lat]
    ...
    identifier = 'G16_NTCS07_*.tif'
    composite = '_BS1'
    script = python_env + 'python process_g16_b01_rgb_sec.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
#######################################################################################################
# GOES-16 Band 03 from RGB - FULL DISK
#######################################################################################################
if g16_b03rgb_fdk:
    max_files = 2
    resolution = 8 # Max Res.: 2 km 
    ...
    identifier = 'G16_DLCS07_*.tif'
    composite = '_B03'
    script = python_env + 'python process_g16_b03_b05_rgb_fdk.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)

#######################################################################################################
# GOES-16 Band 03 from RGB - USER SECTOR
#######################################################################################################
if g16_b03rgb_sec:
    max_files = 2
    extent = [-63.0, -35.0, -35.0, -10.0] # [min_lon, min_lat, max_lon, max_lat]
    ...
    identifier = 'G16_DLCS07_*.tif'
    composite = '_BS3'
    script = python_env + 'python process_g16_b03_b05_rgb_sec.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
#######################################################################################################
# GOES-16 Band 05 from RGB - FULL DISK
#######################################################################################################
if g16_b05rgb_fdk:
    max_files = 2
    resolution = 8 # Max Res.: 2 km 
...
#######################################################################################################
# GOES-16 Band 05 from RGB - USER SECTOR
#######################################################################################################
if g16_b05rgb_sec:
    max_files = 2
    extent = [-63.0, -35.0, -35.0, -10.0] # [min_lon, min_lat, max_lon, max_lat]
...
#######################################################################################################
# GOES-16 and GOES-17 Band 13 Mosaic - USER SECTOR
#######################################################################################################
if g1X_b13mos_sec:
    max_files = 2
    extent = [-120.0, 0.0, -75.0, 45.0] # [min_lon, min_lat, max_lon, max_lat]
    ...
    identifier = '*L2-CMIPF-M*C13_G17*.nc'
    composite = '_MOS'
    script = python_env + 'python process_g1X_b13_mosaic_sec.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
if mtp_glbsst_sec:
    ...
    resolution = 16 # Max Res.: 6 km  
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MTOP-GLBSST*'
    composite = ''
    script = python_env + 'python process_mtp_gblsst_sec.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
if mul_btpwpr_sec:
    max_files = 1
    extent = [-180.0, -71.24, 180.0, 71.24]
    resolution = 8 # Max Res.: 16 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//BTPW//'
    identifier = 'NPR.COMP.TPW*'
    composite = ''
    script = python_env + 'python process_jps_btpw.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# Hourly Global Blended Total Precipitable Water Anomaly
############################################################
if mul_bpctpr_sec:
    max_files = 1
    extent = [-180, -71.24, 180.0, 71.24]
    resolution = 8 # Max Res.: 16 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//BTPW//'
    identifier = 'NPR.COMP.PCT*'
    composite = ''
    script = python_env + 'python process_jps_btpw.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)   
...
if jps_dnband_sec:
    max_files = 80
    extent = [-85.00, -35.00, -30.00, 10.00]
    resolution = 4 # Max Res.: 0.750 km
    prod_dir = gnc_dir + 'JPSS//BANDS//DNB//'
    identifier = 'VIIRS_NCC_EDR_npp_*'
    composite = ''
    script = python_env + 'python process_jps_bands.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
############################################################
# GCOM-W1 - IMAGERY
############################################################
if gcm_imager_sec:
    max_files = 3
    extent = [-156.0, -60.0, 6.00, 60.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-IMAGERY//'
    identifier = 'AMSR2-MBT*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
############################################################
# GCOM-W1 - SOIL MOISTURE and LAND COVER 
############################################################
if gcm_soilmo_sec:
    max_files = 3
    extent = [-156.0, -60.0, 6.00, 60.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-SOILMOISTURE//'
    identifier = 'AMSR2-SOIL*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# GCOM-W1 - CLW, SST, TPW, WSPD
############################################################
if gcm_oceanp_sec:
    max_files = 3
    extent = [-156.00, -65.00, 6.00, 65.00]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-OCEAN//'
    identifier = 'AMSR2-OCEAN*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# GCOM-W1 - RAIN RATE, CONVECTIVE PREC, PROBABILITY
############################################################
if gcm_precip_sec:
    max_files = 3
    extent = [-156.00, -65.00, 6.00, 65.00]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-PRECIPITATION//'
    identifier = 'AMSR2-PRECIP*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
if gcm_icepro_sec:
    ...
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-SEAICE//'
    identifier = 'AMSR2-SEAICE-NH*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
if gcm_icepro_sec:
    ...
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-SEAICE//'
    identifier = 'AMSR2-SEAICE-SH*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# GCOM-W1 - SNOW COVER, SNOW DEPTH, SWE
############################################################
if gcm_snowpr_sec:
    max_files = 3
    extent = [-156.00, 0.00, -45.00, 75.00]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'JPSS//PRODUCTS//G-SNOW//'
    identifier = 'AMSR2-SNOW*'
    composite = ''
    script = python_env + 'python process_gcm_imagery_products.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
...
############################################################
# Sea Ice Type (Northern Hemisphere)
############################################################
if mul_sitype_sec: 
    max_files = 1
    extent = [-180, 30.0, 180.0, 90.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_NH_TYPEn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# Sea Ice Type (Southern Hemisphere)
############################################################
if mul_sitype_sec: 
    max_files = 1
    extent = [-180, -90.0, 180.0, -30.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_SH_TYPEn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# Sea Ice Edge (Northern Hemisphere)
############################################################
if mul_siedge_sec:
    max_files = 1
    extent = [-180, 30.0, 180.0, 90.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_NH_EDGEn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# Sea Ice Edge (Southern Hemisphere)
############################################################
if mul_siedge_sec:
    max_files = 1
    extent = [-180, -90.0, 180.0, -30.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_SH_EDGEn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# DMSP SSMIS Sea Ice Concentration (Northern Hemisphere)
############################################################
if dms_siconc_sec: 
    max_files = 1
    extent = [-180, 30.0, 180.0, 90.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_NH_CONCn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# DMSP SSMIS Sea Ice Concentration (Southern Hemisphere)
############################################################
if dms_siconc_sec: 
    max_files = 1
    extent = [-180, -90.0, 180.0, -30.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*MULT-GL_SH_CONCn*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
      
############################################################
# DMSP AMSU / SSMIS Sea Ice Emissivity (Northern Hemisphere)
############################################################
if dms_siemis_sec:
    max_files = 1
    extent = [-180, 30.0, 180.0, 90.0]
    resolution = 10 # Max Res.: 10 km
    prod_dir = gnc_dir + 'EUMETSAT//'
    identifier = '*DMSP-GL_NH_EMIS*.gz'
    composite = ''
    script = python_env + 'python process_ice.py'
    procProduct(prod_dir, identifier, composite, script, extent[0], extent[1], extent[2], extent[3], resolution)
    
############################################################
# DMSP AMSU / SSMIS Sea Ice Emissivity (Southern Hemisphere)
############################################################
if dms_siemis_sec:
    max_files = 1
    extent = [-180, -90.0, 180.0, -30.0]
