...
# Get the Band 12, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch12 = re.sub('Band08\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
else:
    path_ch12 = re.sub('Band08/OR_ABI-L2-CMIPF*(.*)', '', path_ch08)
...
file = []
for filename in sorted(glob.glob(path_ch12+'Band12//OR_ABI-L2-CMIPF-M*C12_G1*.nc')):
    file.append(filename)
...
# Call the reprojection funcion
grid = remap(path_ch12, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
...
"title_text": "GOES-" + satellite[1:3] + " ARM RGB ", "title_size": int(data_ch08.shape[1] * 0.005), "title_x_offset": int(data_ch08.shape[1] * 0.01), "title_y_offset": data_ch08.shape[0] - int(data_ch08.shape[0] * 0.016), 
