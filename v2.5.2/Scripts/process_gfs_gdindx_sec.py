...
__credits__ = ["Diego Souza", "Regina Ito", "Juan Jose Amides Figueroa", "Jose Galvez"]
__license__ = "GPL"
__version__ = "2.5.2"
...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        "title_text": "GFS (0.5°): Galvez Davison Index (GDI)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        # Plot the image
        img1 = ax.contourf(lons, lats, GDIc, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='both', zorder=1)
        img2 = ax.contour(lons, lats, GDIc, transform=ccrs.PlateCarree(), colors='black', linewidths=0.1, levels=levels, zorder=2)
        ...
        cb.set_label(label='Galvez Davison Indez (GDI)', size='10', weight='bold')
