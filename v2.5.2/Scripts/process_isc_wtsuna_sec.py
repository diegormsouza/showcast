...
my_file = Path(main_dir + '//Logs//iscs-wtsuna_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt')

if not my_file.is_file():
    try:
        os.remove(out_dir + 'iscs-wtsuna.txt')
        # If the iscs output file doesn't exist yet, create one
        file = open(out_dir + 'iscs-wtsuna.txt', 'a')
        file.close()
        # Write the final file title
        with open(out_dir + 'iscs-wtsuna.txt', 'a') as main_file:             
            main_file.write('---------------------------------------------------------------------------') 
            main_file.write('\n')
            main_file.write('\n')
            main_file.write("                             TSUNAMI MESSAGES")
            main_file.write('\n')
            main_file.write('\n')
...
file = open(main_dir + '//Logs//iscs-wtsuna_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a')
file.close()

# Put all file names on the iscs log in a list
log = []
with open(main_dir + '//Logs//iscs-wtsuna_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt') as f:
    log = f.readlines()
f.close()
...
if (new_messages > 0):
    ...
    root.withdraw()
    messagebox.showinfo("TSUNAMI Warning:", "You have " + str(new_messages) + " new message(s)!")
    root.destroy()
    '''
...
'''
    ...
    try:
        ...
        Message(title="TSUNAMI Warning:", message="You have " + str(new_messages) + " new message(s)!", master=root).show()
    except TclError:
        pass
'''
#---------------------------------------------------------------------------------------------
...
with open(main_dir + '//Logs//iscs-wtsuna_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a') as f:
