...
# Get the product type (NDA, NDC, EVC)
prod = path[-3:]
...
if (prod == 'NDA'):
    # Extract the data
    data = file.variables['NDVI_TOA'][ latui:latli , lonli:lonui ]
    prod_title = 'JPSS VIIRS Normalized Difference Vegetation Index at Top of Atmosphere (TOA) 4km'
    label = 'Normalized Difference Vegetation Index at Top of Atmosphere (TOA)'
    satellite = 'JPS'
    product = 'NDVITA'
if (prod == 'NDC'):
    # Extract the data
    data = file.variables['NDVI_TOC'][ latui:latli , lonli:lonui ]
    prod_title = 'JPSS VIIRS Normalized Difference Vegetation Index at Top of Canopy (TOC) 4km'
    label = 'Normalized Difference Vegetation Index at Top of Canopy (TOC)'
    satellite = 'JPS'
    product = 'NDVITC'
if (prod == 'EVC'):
    # Extract the data
    data = file.variables['EVI_TOC'][ latui:latli , lonli:lonui ]
    prod_title = 'JPSS VIIRS Enhanced Vegetation Index at Top of Canopy (TOC) 4km'
    label = 'Enhanced Vegetation Index at Top of Canopy (TOC)'
    satellite = 'JPS'
    product = 'EVITOC'
...
"dpi": 150, 
"states_color": 'black', "states_width": sizey * factor * 0.00006, 
"countries_color": 'black', "countries_width": sizey * factor * 0.00012,
"continents_color": 'black', "continents_width": sizey * factor * 0.00025,
"grid_color": 'white', "grid_width": sizey * factor * 0.00025, "grid_interval": 10.0,
"vmin": vmin, "vmax": vmax, "cmap": cmap,
"title_text": prod_title + " ", "title_size": int(sizex * factor * 2 * 0.0025), "title_x_offset": int(sizex * factor * 0.01), "title_y_offset": sizey * factor - int(sizey * factor * 0.016),
"thick_interval": thick_interval, "cbar_labelsize": int(sizey * factor * 0.005), "cbar_labelpad": -int(sizey * factor * 0.0),
...
fig = plt.figure(figsize=(sizex * factor/float(plot_config["dpi"]), sizey * factor/float(plot_config["dpi"])), dpi=plot_config["dpi"])
...
cb.set_label(label=label, color='black', size=plot_config["title_size"], weight='bold')
