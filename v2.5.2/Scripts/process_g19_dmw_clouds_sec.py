...
"title_text": "GOES-19 DWM + BAND-14", "title_size": int(data.shape[1] * 0.005), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
"thick_interval": 0, "cbar_labelsize": int(data.shape[1] * 0.005), "cbar_labelpad": -int(data.shape[1] * 0.0),
...
for x in range(1, 7):
    ...
    # Make the barb plot  
    ax.barbs(lons, lats, u_comp, v_comp, length = 5.0, sizes = dict(emptybarb=0.25, spacing=0.2, height=0.5), linewidth=0.8, pivot='middle', barbcolor=color, transform=ccrs.PlateCarree(), zorder=6)
...
my_legend = plt.imread(main_dir + '//Legends//DMWF_legend.png')
