...
my_file = Path(main_dir + '//Logs//iscs-atrodn_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt')

if not my_file.is_file():
    try:
        os.remove(out_dir + 'iscs-atrodn.txt')
        # If the iscs output file doesn't exist yet, create one
        file = open(out_dir + 'iscs-atrodn.txt', 'a')
        file.close()
        # Write the final file title
        with open(out_dir + 'iscs-atrodn.txt', 'a') as main_file:             
            main_file.write('---------------------------------------------------------------------------') 
            main_file.write('\n')
            main_file.write('\n')
            main_file.write("         TROPICAL WEATHER DISCUSSION (NORTH ATLANTIC AREA) MESSAGES        ")
            main_file.write('\n')
            main_file.write('\n')
...
file = open(main_dir + '//Logs//iscs-atrodn_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a')
file.close()

# Put all file names on the iscs log in a list
log = []
with open(main_dir + '//Logs//iscs-atrodn_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt') as f:
    log = f.readlines()
f.close()
...
'''
    ...
    try:
        ...
        Message(title="TROPICAL WEATHER DISCUSSION (North Atlantic) Warning:", message="You have " + str(new_messages) + " new message(s)!", master=root).show()
    except TclError:
        pass
'''
#---------------------------------------------------------------------------------------------
...
with open(main_dir + '//Logs//iscs-atrodn_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a') as f:
