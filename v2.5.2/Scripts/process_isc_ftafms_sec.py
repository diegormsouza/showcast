...
my_file = Path(main_dir + '//Logs//iscs-ftafms_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt')

if not my_file.is_file():
    try:
        os.remove(out_dir + 'iscs-ftafms.txt')
        # If the iscs output file doesn't exist yet, create one
        file = open(out_dir + 'iscs-ftafms.txt', 'a')
        file.close()
        # Write the final file title
        with open(out_dir + 'iscs-ftafms.txt', 'a') as main_file:             
            main_file.write('---------------------------------------------------------------------------') 
            main_file.write('\n')
            main_file.write('\n')
            main_file.write("                              TAF MESSAGES")
            main_file.write('\n')
            main_file.write('\n')
            main_file.write("T1T2: ")
            main_file.write('FC and FT')
            main_file.write('\n')
...
file = open(main_dir + '//Logs//iscs-ftafms_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a')
file.close()

# Put all file names on the iscs log in a list
log = []
with open(main_dir + '//Logs//iscs-ftafms_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt') as f:
    log = f.readlines()
f.close()
...
'''
    ...
    try:
        ...
        Message(title="TAF Warning:", message="You have " + str(new_messages) + " new message(s)!", master=root).show()
    except TclError:
        pass
'''
#---------------------------------------------------------------------------------------------
...
with open(main_dir + '//Logs//iscs-ftafms_log_' + date[0:4] + "-" + date[4:6] + "-" + date[6:8] + '.txt', 'a') as f:
