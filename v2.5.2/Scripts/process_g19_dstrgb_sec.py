...
# Get the Band 14, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch14 = re.sub('Band15\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch15)
else:
    path_ch14 = re.sub('Band15/OR_ABI-L2-CMIPF*(.*)', '', path_ch15)
...
file = []
for filename in sorted(glob.glob(path_ch14+'Band14//OR_ABI-L2-CMIPF-M*C14_G1*.nc')):
    file.append(filename)
...
osystem = platform.system()
if osystem == "Windows":
    path_ch11 = re.sub('Band15\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch15)
else:
    path_ch11 = re.sub('Band15/OR_ABI-L2-CMIPF*(.*)', '', path_ch15)
...
data_ch15 = grid.ReadAsArray() - 273.15
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Read the image
...
#------------------------------------------------------------------------------------------------------
# Read the image
file_ch14 = Dataset(path_ch14)

# Variable to remap
variable = "CMI"

# Call the reprojection funcion
grid = remap(path_ch14, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2)
     
# Read the data returned by the function 
data_ch14 = grid.ReadAsArray() - 273.15
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Read the image
...
"dpi": 150, 
"states_color": 'white', "states_width": data_ch15.shape[0] * 0.00018, 
...
"title_text": "GOES-" + satellite[1:3] + " DST RGB ", "title_size": int(data_ch15.shape[1] * 0.005), "title_x_offset": int(data_ch15.shape[1] * 0.01), "title_y_offset": data_ch15.shape[0] - int(data_ch15.shape[0] * 0.016), 
