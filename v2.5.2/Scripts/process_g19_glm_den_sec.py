...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        "title_text": "GOES-19 GLM (Density)", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        glm = ax.imshow(heatmap, vmin=0, vmax=60, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=ccrs.PlateCarree(), alpha = 1.00, zorder=4)
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.5, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=11)
