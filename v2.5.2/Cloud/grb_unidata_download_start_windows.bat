...
IF EXIST %ParentFolderName%\Miniconda3\envs\showcast\ (
	...
	echo %ParentFolderName%\Miniconda3\envs\showcast\python.exe %~dp0Scripts\grb_unidata_download_start.py
	call %ParentFolderName%\Miniconda3\envs\showcast\python.exe %~dp0Scripts\grb_unidata_download_start.py
) ELSE (
