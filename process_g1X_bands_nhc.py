...
#---------------------------------------------------------------------------------------------    
# Plot configuration
if int(band) == 1 or int(band) == 2 or int(band) == 3 or int(band) == 5:
    plot_config = {
    "resolution": band_resolution_km, 
    "dpi": 150, 
    "states_color": 'gold', "states_width": data.shape[0] * 0.000125, 
    "countries_color": 'gold', "countries_width": data.shape[0] * 0.000125,
    "continents_color": 'gold', "continents_width": data.shape[0] * 0.000125,
    "grid_color": 'white', "grid_width": data.shape[0] * 0.000125, "grid_interval": 5.0,
    "vmin": vmin, "vmax": vmax, "cmap": cmap,
    "title_text": "GOES-" + satellite[1:3] + " Band " + band + " + NHC ", "title_size": int(data.shape[1] * 0.00125), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
    "thick_interval": thick_interval, "cbar_labelsize": int(data.shape[0] * 0.0025), "cbar_labelpad": -int(data.shape[0] * 0),
    ...
    "resolution": band_resolution_km, 
    "dpi": 150, 
    "states_color": 'black', "states_width": data.shape[0] * 0.00025, 
    "countries_color": 'black', "countries_width": data.shape[0] * 0.00025,
    "continents_color": 'black', "continents_width": data.shape[0] * 0.00025,
    "grid_color": 'white', "grid_width": data.shape[0] * 0.00025, "grid_interval": 5.0,
    "vmin": vmin, "vmax": vmax, "cmap": cmap,
    "title_text": "GOES-" + satellite[1:3] + " Band " + band + " + NHC ", "title_size": int(data.shape[1] * 0.0025), "title_x_offset": int(data.shape[1] * 0.02), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
...
for name in tsystems:
    ...
    if storm.invest == True:
        ...
        ax.add_geometries(areas_low, ccrs.PlateCarree(), edgecolor='gold',facecolor='yellow',alpha=0.2, linewidth=plot_config["states_width"]*2, zorder=4)
        ax.add_geometries(areas_medium, ccrs.PlateCarree(), edgecolor='darkorange',facecolor='orange',alpha=0.2, linewidth=plot_config["states_width"]*2, zorder=4)
        ax.add_geometries(areas_high, ccrs.PlateCarree(), edgecolor='darkred',facecolor='red',alpha=0.2, linewidth=plot_config["states_width"]*2, zorder=4)
        # Add positions
        ...
        for pl in points_low:
            ax.scatter(pl.geometry.x,pl.geometry.y, edgecolor='black', facecolor='gold', transform=ccrs.PlateCarree(), zorder=5)
            ax.text(pl.geometry.x+0.5,pl.geometry.y+0.5,pl.attributes['PROB2DAY'],color='gold', fontsize='large', fontweight='bold',zorder=5)
        for pm in points_medium:
            ax.scatter(pm.geometry.x,pm.geometry.y, edgecolor='black', facecolor='orange', transform=ccrs.PlateCarree(), zorder=5)
            ax.text(pm.geometry.x+0.5,pm.geometry.y+0.5,pm.attributes['PROB2DAY'],color='orange', fontsize='large', fontweight='bold',zorder=5)
        for ph in points_high:
            ax.scatter(ph.geometry.x,ph.geometry.y, edgecolor='black', facecolor='red', transform=ccrs.PlateCarree(), zorder=5)
            ax.text(ph.geometry.x+0.5,ph.geometry.y+0.5,ph.attributes['PROB2DAY'],color='red', fontsize='large', fontweight='bold',zorder=5)
    else:
...
for label, xpt, ypt, x_offset, y_offset, size, col, mtype, mcolor, msize in zip(labels, city_lons, city_lats, x_offsets, y_offsets, sizes, colors, marker_types, marker_colors, marker_sizes):
    ax.plot(xpt, ypt, str(mtype), color=str(mcolor), markersize=int(msize), transform=ccrs.Geodetic(), markeredgewidth=1.0, markeredgecolor=(0, 0, 0, 1), zorder=8)
    txt = ax.text(xpt+x_offset , ypt+y_offset, label, fontsize=int(size), fontweight='bold', color=str(col), transform=ccrs.Geodetic(), zorder=9)
...
newax = fig.add_axes([0.01, 0.04, 0.10, 0.10], anchor='SW', zorder=10) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
...
cb.ax.tick_params(axis='x', colors='lightgrey', labelsize=plot_config["cbar_labelsize"])
#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
