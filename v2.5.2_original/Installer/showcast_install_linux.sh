...
if [[ $REPLY =~ ^[Yy]$ ]]; then
	...
	echo
	$parentdir/Miniconda3/condabin/conda install mamba -n base -c conda-forge --yes
	...
	if [[ $REPLY =~ ^[Yy]$ ]]; then
		...
		if [ -d "$parentdir/Miniconda3/envs/showcast" ]; then
			...
			# if not, update conda and create the showcast env with mamba
			echo
			...
			echo
			$parentdir/Miniconda3/condabin/mamba env create -f $parentdir/Installer/Miniconda3/environment.yml
		fi		
	fi
fi
