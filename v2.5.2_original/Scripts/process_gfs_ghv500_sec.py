...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        "title_text": "GFS (0.5°): Relative Vorticity (1 e-5 / s) + Geopotential Height (500 hPa)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016),  
        ...
        # Plot the image
        img1 = ax.contourf(lons, lats, rv500, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='both', zorder=1)
        ...
        # Plot the image
        img3 = ax.contour(lons, lats, gh500, transform=ccrs.PlateCarree(), colors='black', linewidths=2.0, levels=levels, zorder=2)
        ax.clabel(img3, inline=1, inline_spacing=0, fontsize=10,fmt = '%1.0f', colors= 'black', zorder=3)
        ...
        # Add gridlines
        gl = gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=True, zorder=7)
        ...
        cb.set_label(label='Relative Vorticity (1 e-5 / s)', size='10', weight='bold')
