...
path_ch10 = path_ch10[:-4]

# Get the Band 15, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch08 = re.sub('Band10\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch10)
else:
    path_ch08 = re.sub('Band10/OR_ABI-L2-CMIPF*(.*)', '', path_ch10)

# Get the start of scan from the file name
scan_start = (path_ch10[path_ch10.find("_s")+2:path_ch10.find("_e")])

file = []
for filename in sorted(glob.glob(path_ch08+'Band08//OR_ABI-L2-CMIPF-M*C08_G1*.nc')):
...
if not matching:
    ...
    path_ch08 = file[index]

# Get the Band 15, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch13 = re.sub('Band10\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch10)
else:
    path_ch13 = re.sub('Band10/OR_ABI-L2-CMIPF*(.*)', '', path_ch10)
...
# Read the satellite 
satellite = getattr(file_ch10, 'platform_ID')

# Read the band number
band = str(file_ch10.variables['band_id'][0]).zfill(2)
...
longitude = file_ch10.variables['goes_imager_projection'].longitude_of_projection_origin

# Read the semi major axis
a = file_ch10.variables['goes_imager_projection'].semi_major_axis

# Read the semi minor axis
b = file_ch10.variables['goes_imager_projection'].semi_minor_axis

# Calculate the image extent 
h = file_ch10.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch10.variables['x_image_bounds'][0] * h 
x2 = file_ch10.variables['x_image_bounds'][1] * h 
y1 = file_ch10.variables['y_image_bounds'][1] * h 
y2 = file_ch10.variables['y_image_bounds'][0] * h 

# Getting the file time and date
add_seconds = int(file_ch10.variables['time_bounds'][0])
...
"dpi": 150, 
"states_color": 'white', "states_width": data_ch10.shape[0] * 0.00018, 
"countries_color": 'white', "countries_width": data_ch10.shape[0] * 0.00012,
"continents_color": 'white', "continents_width": data_ch10.shape[0] * 0.00025,
"grid_color": 'white', "grid_width": data_ch10.shape[0] * 0.00025, "grid_interval": 10.0,
"vmin": 0, "vmax": 1, "cmap": 'jet',
"title_text": "GOES-" + satellite[1:3] + " SWV RGB ", "title_size": int(data_ch10.shape[1] * 0.005), "title_x_offset": int(data_ch10.shape[1] * 0.01), "title_y_offset": data_ch10.shape[0] - int(data_ch10.shape[0] * 0.016), 
"thick_interval": 0, "cbar_labelsize": int(data_ch10.shape[0] * 0.005), "cbar_labelpad": -int(data_ch10.shape[0] * 0.0),
"file_name_id_1": satellite,  "file_name_id_2": product
}
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Choose the plot size (width x height, in inches)
fig = plt.figure(figsize=(data_ch10.shape[1]/float(plot_config["dpi"]), data_ch10.shape[0]/float(plot_config["dpi"])), dpi=plot_config["dpi"])
...
my_legend = plt.imread(main_dir + '//Legends//SWV_legend.png')
