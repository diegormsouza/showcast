...
def procDsif(variable,product):
    ...
    if (variable == 'CAPE'):  
        ...
        # LI
        colors = ["#4e0000", "#fc0004", "#ddd804", "#727204", "#413e68", "#7d7be4", "#d2a572", "#3a270f"]
        ...
        # TT
        colors = ["#3a290d", "#daa67f", "#9091f4", "#504c97", "#0b6661", "#74a42c", "#98c231", "#ffff00", "#f67c5d", "#902a29", "#6b035f", "#ee00f3", "#eebb89", "#eebbed"]
        cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", colors)
        vmin = 0
        vmax = 70
        thick_interval = 5
    ...
    "resolution": 8, 
    "dpi": 150, 
    "states_color": 'white', "states_width": data.shape[0] * factor * 0.00018, 
    "countries_color": 'gold', "countries_width": data.shape[0] * factor * 0.00012,
    "continents_color": 'gold', "continents_width": data.shape[0] * factor * 0.00025,
    "grid_color": 'white', "grid_width": data.shape[0] * factor * 0.00025, "grid_interval": 10.0,
    "vmin": vmin, "vmax": vmax, "cmap": cmap,
    "title_text": "GOES-19 " + prod_name + " " + variable, "title_size": int(data.shape[0] * factor * 0.005), "title_x_offset": int(data.shape[1] * factor * 0.01), "title_y_offset": data.shape[0] * factor - int(data.shape[0] * factor * 0.016), 
