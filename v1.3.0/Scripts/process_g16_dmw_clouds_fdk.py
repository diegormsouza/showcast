...
"title_text": "GOES-16 DWM + BAND-14", "title_size": int(data.shape[1] * 0.005), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
...
for x in range(1, 7):
    ...
    # Make the barb plot  
    ax.barbs(lons, lats, u_comp, v_comp, length = 2.50, sizes = dict(emptybarb=0.125, spacing=0.1, height=0.250), linewidth=0.5, pivot='middle', barbcolor=color, transform=ccrs.PlateCarree(), zorder=6)
...
for label, xpt, ypt, x_offset, y_offset, size, col, mtype, mcolor, msize in zip(labels, city_lons, city_lats, x_offsets, y_offsets, sizes, colors, marker_types, marker_colors, marker_sizes):
    ax.plot(xpt, ypt, str(mtype), color=str(mcolor), markersize=int(msize), transform=ccrs.Geodetic(), zorder=12)
...
my_legend = plt.imread('..//Legends//DMWF_legend.png')
