...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        "title_text": "GFS (0.5°): 2 m Temperature (°C)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=False, zorder=8)
        ...
        cb.set_label(label='2 m Temperature (°C)', size=plot_config["cbar_labelsize"], weight='bold')
