...
#------------------------------------------------------------------------------------------------------
# Remap the GeoTIFF
grid2 = gdal.Warp(path_out + '_rep', path_out, format = 'GTiff', outputType = gdal.GDT_Float32, srcSRS='EPSG:4326', dstSRS ='+proj=geos +h=35786000 +a=6378140 +b=6356750 +lon_0=-75 +sweep=x')
grid2 = None
...
#data2 = (R * (1.0 / 255.0))
#data3 = (((G * (1.0 / 255.0)) ** 2) - (((data1 + data2)/2) * 0.93)) / 0.07 
#data3 = ((G * (1.0 / 255.0)) - (0.48358168 * np.sqrt(data2)) - (0.45706946 * np.sqrt(data1))) / 0.06038137
#data3 = data3 * data3
...
# Reference file
image = main_dir + '//Utils//GOES_coord_ref//coordinate_reference.nc'
...
X = np.linspace(X.min(), X.max(), R.shape[1])
Y = file.variables['y'][:][::4] * sat_h
Y = np.linspace(Y.min(), Y.max(), R.shape[0])
...
mask = (data == [R[0,0],G[0,0],B[0,0]]).all(axis=2)
data[mask] = np.nan
