...
print('MSG SST EUR Script started.')
start_time = t.time()
...
satellite = 'MTP'
# product = 'SSTSKE'
# storage =  product + '_STE' # to store data before mosaicing
#
...
#
# # Create the mosaic ===========================================================
# gdal.BuildVRT(out_dir + 'MTP_SST_STE_' + date + '.vrt', sorted(glob.glob(out_dir + satellite + "_" + storage + "_" + date_month +'*.tif'), key=os.path.getmtime), srcNodata = -32768)
# gdal.Translate(out_dir + 'MTP_SST_STE_' + date + '.tif', out_dir + 'MTP_SST_STE_' + date + '.vrt')
# print('Generated GeoTIFF: ', 'MTP_SST_STE_' + date + '.tif')
# os.remove(out_dir + 'MTP_SST_STE_' + date + '.vrt')
# # Export the result to GeoTIFF ================================================
#
# # Read the mosaic
# mosaic = gdal.Open(out_dir + 'MTP_SST_STE_' + date + '.tif')
...
"title_text": "MSG " + prod_title + " ", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.05), "title_y_offset": sizey - int(sizey * 0.025),
"thick_interval": thick_interval, "cbar_labelsize": int(sizey * 0.007), "cbar_labelpad": -int(sizey * 0.0),
"file_name_id_1": "MTP",  "file_name_id_2": product + "_EU"
}
...
# ax.imshow(imread(fname), origin='upper', transform=ccrs.PlateCarree(), extent=[-90, 90, -80, 80], zorder=1)
# ax.imshow(imread(fname), origin='upper', transform=ccrs.PlateCarree(), extent=[extent[0], extent[2], extent[1], extent[3]], zorder=1)
