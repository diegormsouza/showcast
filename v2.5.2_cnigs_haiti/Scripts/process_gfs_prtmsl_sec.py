...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        "title_text": "GFS (0.5°): PMSL + Winds (850 hPa)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": int(sizex * 0.005), "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        # Plot the image
        img1 = ax.contourf(lons, lats, prmls, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='both', zorder=1)
        img2 = ax.contour(lons, lats, prmls, transform=ccrs.PlateCarree(), colors='black', linewidths=0.1, levels=levels, zorder=2)
        ...
        img3 = ax.barbs(lons[::6,::6], lats[::6,::6], ucomp[::6,::6], vcomp[::6,::6], length = 5.0, sizes = dict(emptybarb=0.0, spacing=0.2, height=0.5), linewidth=0.8, pivot='middle', barbcolor='gray', transform=ccrs.PlateCarree(), zorder=4)
        ...
        # Add gridlines
        gl = gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=True, zorder=8)
        ...
        cb.set_label(label='PSML (hPa)', size='10', weight='bold')
