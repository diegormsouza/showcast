...
path_ch06 = path_ch06[:-4]

# Get the Band 03, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch03 = re.sub('Band06\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch06)
else:
    path_ch03 = re.sub('Band06/OR_ABI-L2-CMIPF*(.*)', '', path_ch06)

# Get the start of scan from the file name
scan_start = (path_ch06[path_ch06.find("_s")+2:path_ch06.find("_e")])
...
osystem = platform.system()
if osystem == "Windows":
    path_ch02 = re.sub('Band06\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch06)
else:
    path_ch02 = re.sub('Band06/OR_ABI-L2-CMIPF*(.*)', '', path_ch06)
...
# Read the satellite 
satellite = getattr(file_ch06, 'platform_ID')

# Read the band number
band = str(file_ch06.variables['band_id'][0]).zfill(2)
...
longitude = file_ch06.variables['goes_imager_projection'].longitude_of_projection_origin

# Read the semi major axis
a = file_ch06.variables['goes_imager_projection'].semi_major_axis

# Read the semi minor axis
b = file_ch06.variables['goes_imager_projection'].semi_minor_axis

# Calculate the image extent 
h = file_ch06.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch06.variables['x_image_bounds'][0] * h 
x2 = file_ch06.variables['x_image_bounds'][1] * h 
y1 = file_ch06.variables['y_image_bounds'][1] * h 
y2 = file_ch06.variables['y_image_bounds'][0] * h 

# Getting the file time and date
add_seconds = int(file_ch06.variables['time_bounds'][0])
...
# Satellite height
sat_h = file_ch06.variables['goes_imager_projection'].perspective_point_height
# Satellite longitude
sat_lon = file_ch06.variables['goes_imager_projection'].longitude_of_projection_origin
# Satellite sweep
sat_sweep = file_ch06.variables['goes_imager_projection'].sweep_angle_axis
# Read the resolution
band_resolution_km = getattr(file_ch06, 'spatial_resolution')
band_resolution_km = float(band_resolution_km[:band_resolution_km.find("km")])
# Division factor to reduce image size
...
X = file_ch06.variables['x'][:][::f] * sat_h
Y = file_ch06.variables['y'][:][::f] * sat_h
...
sun_zenith = np.zeros((data_ch06.shape[0], data_ch06.shape[1]))
...
"dpi": 150, 
"states_color": 'white', "states_width": data_ch06.shape[0] * 0.00006, 
"countries_color": 'white', "countries_width": data_ch06.shape[0] * 0.00012,
"continents_color": 'white', "continents_width": data_ch06.shape[0] * 0.00025,
"grid_color": 'white', "grid_width": data_ch06.shape[0] * 0.00025, "grid_interval": 10.0,
"vmin": 0, "vmax": 1, "cmap": 'jet',
"title_text": "GOES-" + satellite[1:3] + " DLF RGB ", "title_size": int(data_ch06.shape[1] * 0.005), "title_x_offset": int(data_ch06.shape[1] * 0.01), "title_y_offset": data_ch06.shape[0] - int(data_ch06.shape[0] * 0.016), 
"thick_interval": 0, "cbar_labelsize": int(data_ch06.shape[0] * 0.005), "cbar_labelpad": -int(data_ch06.shape[0] * 0.0),
"file_name_id_1": satellite,  "file_name_id_2": product
}
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# Choose the plot size (width x height, in inches)
fig = plt.figure(figsize=(data_ch06.shape[1]/float(plot_config["dpi"]), data_ch06.shape[0]/float(plot_config["dpi"])), dpi=plot_config["dpi"])
