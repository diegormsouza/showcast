...
path_ch03 = path_ch03[:-4]

# Get the Band 05, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch05 = re.sub('Band03\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch03)
else:
    path_ch05 = re.sub('Band03/OR_ABI-L2-CMIPF*(.*)', '', path_ch03)

# Get the start of scan from the file name
scan_start = (path_ch03[path_ch03.find("_s")+2:path_ch03.find("_e")])
...
# Get the Band 07, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch07 = re.sub('Band03\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch03)
else:
    path_ch07 = re.sub('Band03/OR_ABI-L2-CMIPF*(.*)', '', path_ch03)
...
osystem = platform.system()
if osystem == "Windows":
    path_ch13 = re.sub('Band03\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch03)
else:
    path_ch13 = re.sub('Band03/OR_ABI-L2-CMIPF*(.*)', '', path_ch03)
...
# Read the satellite 
satellite = getattr(file_ch03, 'platform_ID')

# Read the band number
band = str(file_ch03.variables['band_id'][0]).zfill(2)

# Desired resolution
resolution = int(sys.argv[6])

# Read the resolution
band_resolution_km = getattr(file_ch03, 'spatial_resolution')
...
longitude = file_ch03.variables['goes_imager_projection'].longitude_of_projection_origin

# Read the semi major axis
a = file_ch03.variables['goes_imager_projection'].semi_major_axis

# Read the semi minor axis
b = file_ch03.variables['goes_imager_projection'].semi_minor_axis

# Calculate the image extent 
h = file_ch03.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch03.variables['x_image_bounds'][0] * h 
x2 = file_ch03.variables['x_image_bounds'][1] * h 
y1 = file_ch03.variables['y_image_bounds'][1] * h 
y2 = file_ch03.variables['y_image_bounds'][0] * h 

# Getting the file time and date
add_seconds = int(file_ch03.variables['time_bounds'][0])
...
refl39 = Calculator('GOES-19', 'abi', 'ch7')
data_ch07b = refl39.reflectance_from_tbs(sun_zenith, data_ch07, data_ch13)
...
"title_text": "GOES-" + satellite[1:3] + " DSF RGB ", "title_size": int(data_ch07.shape[1] * 0.005), "title_x_offset": int(data_ch07.shape[1] * 0.01), "title_y_offset": data_ch07.shape[0] - int(data_ch07.shape[0] * 0.016), 
...
my_legend = plt.imread(main_dir + '//Legends//DAYSNOWFOG_legend.png')
